...
const { app, BrowserWindow, Menu, dialog, Tray, nativeImage, ipcMain, globalShortcut } = require('electron');
...
async function start() {
      ...
      },
      {
        ...
        submenu: [
          ...
          },
          {
            label: 'Add Entry',
            click: () => {
              try { openAddEntryWindow('/add-entry'); } catch (e) { console.error(e); }
            }
          }
        ]
      }
  ...
  function openAddEntryWindow(route) {
    try {
      ...
      modal.webContents.on('did-finish-load', () => {
        try {
          modal.webContents.executeJavaScript(`window.location.hash = '${routeFragment}'`);
        } catch (err) {
          // ignore
        }
      });
  ...
  try {
    ...
    const trayMenu = Menu.buildFromTemplate([
      { label: 'Show', click: () => { BrowserWindow.getAllWindows().forEach(w => w.show()); } },
      { label: 'Quit', click: () => { isQuiting = true; app.quit(); } }
    ]);
    tray.setContextMenu(trayMenu);
    tray.setToolTip('mysampleproject');
    tray.on('click', () => { BrowserWindow.getAllWindows().forEach(w => w.show()); });
  } catch (e) {
  ...
  const probeVite = () => new Promise((resolve) => {
    const http = require('http');
    const options = { host: '127.0.0.1', port: 5173, path: '/', method: 'GET', timeout: 1500 };
